...
PreferencesGeneralPage::PreferencesGeneralPage(
  ...
  uiGrid->Add(
      new wxStaticText(uiSizer->GetStaticBox(), wxID_ANY, i18n("language")), 0,
      wxALIGN_CENTER_VERTICAL);
  uiGrid->Add(m_language, 1, wxALIGN_RIGHT | wxALIGN_CENTER_VERTICAL);
  uiGrid->Add(
      new wxStaticText(uiSizer->GetStaticBox(), wxID_ANY, i18n("theme")), 0,
      wxALIGN_CENTER_VERTICAL);
  uiGrid->Add(m_theme, 1, wxALIGN_RIGHT | wxALIGN_CENTER_VERTICAL);
  uiSizer->Add(uiGrid, 1, wxEXPAND | wxALL, FromDIP(5));

  wxStaticBoxSizer *miscSizer =
      new wxStaticBoxSizer(wxVERTICAL, this, i18n("miscellaneous"));
  wxFlexGridSizer *miscGrid = new wxFlexGridSizer(2, FromDIP(10), FromDIP(10));

  m_labelColor =
      new wxCheckBox(miscSizer->GetStaticBox(), wxID_ANY,
                     i18n("use_label_color_as_background_in_torrent_list"));
  m_skipAddTorrentDialog = new wxCheckBox(miscSizer->GetStaticBox(), wxID_ANY,
                                          i18n("skip_add_torrent_dialog"));
  m_autoStart = new wxCheckBox(miscSizer->GetStaticBox(), wxID_ANY,
                               i18n("start_with_windows"));
  ...
  miscGrid->Add(0, 0);
  miscGrid->Add(new wxStaticText(miscSizer->GetStaticBox(), wxID_ANY,
                                 i18n("start_position")),
  ...
  wxStaticBoxSizer *notifSizer =
      new wxStaticBoxSizer(wxVERTICAL, this, i18n("notification_area"));
  wxFlexGridSizer *notifGrid = new wxFlexGridSizer(1, FromDIP(10), FromDIP(10));

  m_showNotificationIcon =
      new wxCheckBox(notifSizer->GetStaticBox(), wxID_ANY,
                     i18n("show_picotorrent_in_notification_area"));
  m_minimizeNotification =
      new wxCheckBox(notifSizer->GetStaticBox(), wxID_ANY,
                     i18n("minimize_to_notification_area"));
  m_closeNotification = new wxCheckBox(notifSizer->GetStaticBox(), wxID_ANY,
                                       i18n("close_to_notification_area"));
  ...
  m_skipAddTorrentDialog->SetValue(
  ...
  m_startPosition->Append(i18n("normal"),
                          new ClientData<Configuration::WindowState>(
                              Configuration::WindowState::Normal));
  m_startPosition->Append(i18n("minimized"),
                          new ClientData<Configuration::WindowState>(
                              Configuration::WindowState::Minimized));
  m_startPosition->Append(i18n("hidden"),
                          new ClientData<Configuration::WindowState>(
                              Configuration::WindowState::Hidden));
  m_startPosition->Append(i18n("maximized"),
                          new ClientData<Configuration::WindowState>(
                              Configuration::WindowState::Maximized));
  m_startPosition->SetSelection(m_cfg->Get<int>("start_position").value());

  // Notification area
  m_showNotificationIcon->SetValue(
      m_cfg->Get<bool>("show_in_notification_area").value());
  m_minimizeNotification->SetValue(
...
void PreferencesGeneralPage::Save(bool *restartRequired) {
  ...
  ClientData<std::string> *langData =
      langIndex >= 0 ? reinterpret_cast<ClientData<std::string> *>(
                           m_language->GetClientObject(langIndex))
                     : nullptr;

  int themeIndex = m_theme->GetSelection();
  ClientData<std::string> *themeData =
      themeIndex >= 0 ? reinterpret_cast<ClientData<std::string> *>(
                            m_theme->GetClientObject(themeIndex))
                      : nullptr;

  int startPosIndex = m_startPosition->GetSelection();
  ClientData<Configuration::WindowState> *startPosData =
  ...
  m_cfg->Set("minimize_to_notification_area",
             m_minimizeNotification->GetValue());
