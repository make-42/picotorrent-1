...
std::wstring SecondsToFriendly(std::chrono::seconds secs) {
  ...
  std::chrono::minutes min_left =
      std::chrono::duration_cast<std::chrono::minutes>(secs - hours_left);
  std::chrono::seconds sec_left =
  ...
  if (hours_left.count() <= 0) {
    ...
    return fmt::format(i18n("eta_ms_format"), min_left.count(),
                       sec_left.count());
  }

  return fmt::format(i18n("eta_hms_format"), hours_left.count(),
...
TorrentDetailsOverviewPanel::TorrentDetailsOverviewPanel(wxWindow *parent,
                                                         wxWindowID id,
                                                         int cols,
                                                         bool showPieceProgress)
    : wxScrolledWindow(parent, id), m_pieceProgress(nullptr),
...
void TorrentDetailsOverviewPanel::Refresh(
  ...
  m_pieces->SetLabel(
      fmt::format(i18n("d_of_d"), status.pieces.count(), status.pieces.size()));
  m_ratio->SetLabel(fmt::format("{:.3f}", status.ratio));
