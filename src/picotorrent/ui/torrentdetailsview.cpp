...
TorrentDetailsView::TorrentDetailsView(
    ...
    : wxNotebook(parent, id), m_cfg(cfg),
...
void TorrentDetailsView::Refresh(
    std::map<lt::info_hash_t, pt::BitTorrent::TorrentHandle *> const
        &torrents)
{
  if (torrents.size() != 1)
  {
...
{
  auto showPieceProgress =
      m_cfg->Get<bool>("ui.torrent_overview.show_piece_progress");
  auto cols = m_cfg->Get<int>("ui.torrent_overview.columns");

  m_overview->UpdateView(cols.value_or(2), showPieceProgress.value_or(true));
}
